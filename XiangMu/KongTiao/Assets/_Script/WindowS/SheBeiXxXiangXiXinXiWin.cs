...
public class SheBeiXxXiangXiXinXiWin : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        SheBeiOnClick(btnSheBei);//默认点击了第一个
    ...
    void ItemButtonOnClick(GameObject btn)
    {
        ...
        for (int i = 0; i < ItemBtn.Length; i++)
        {
            if (btn == ItemBtn[i])
            {
                Debugger.LogError(btn.GetComponent<MoXingNameData>().names);
                inputFileTex.GetComponent<InputField>().text = btn.GetComponent<MoXingNameData>().names;
                Debugger.Log(i);
                numSheBei = i;
            }
        }
